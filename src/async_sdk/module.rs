...
impl ImportModule {
    ...
    pub fn create_wasi(args: &[&str], envs: &[&str], preopens: &[&str]) -> WasmEdgeResult<Self> {
        fn to_cstring_vec(s: &[&str]) -> Vec<CString> {
            let mut r = vec![];
            for s in s {
        ...
        fn csstring_vec_to_ptr(s: &[CString]) -> Vec<*const std::ffi::c_char> {
            let mut r = vec![];
            for cs in s {
                r.push(cs.as_ptr())
            }
        ...
        let args_ptrs = csstring_vec_to_ptr(&args);
        let args_len = args.len();

        let envs = to_cstring_vec(envs);
        let envs_ptrs = csstring_vec_to_ptr(&envs);
        let envs_len = envs.len();

        let preopens = to_cstring_vec(preopens);
        let preopens_ptrs = csstring_vec_to_ptr(&preopens);
    ...
    pub fn add_func<T: Sized>(
        ...
        unsafe {
            let func = Function::create(ty, real_fn, data, cost)?;
